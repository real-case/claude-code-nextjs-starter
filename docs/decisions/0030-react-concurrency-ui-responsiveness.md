...
- the cost is **network latency**, not rendering — then debounce/throttle the request (or
  use TanStack Query's request handling, **0022**), since deferring a render does not reduce
  fetches;
...
* Good, because memoization genuinely reduces re-render and recompute cost — though in this
  project that memoization is applied automatically by the React Compiler (**0033**), not by hand.
...
responsiveness convention to **0022**, which makes optimistic UI the preferred default for
*mutation* latency — **0030** addresses *render-scheduling* latency; together they cover the
two main sources of perceived sluggishness. The rapidly-changing inputs deferred here are
often client UI state held in Zustand (**0023**). Debounce/throttle and list virtualization
...
handled automatically by the React Compiler (**0033**). The compiler memoizes (what to
