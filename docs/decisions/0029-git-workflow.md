...
the CI quality gate runs on pull requests (**0008**), and the hosting platform produces a
preview deployment per branch/PR (**0007**). It must also be simple enough not to impose
...
* **Gated integration** — every change lands via a pull request that the CI gate (**0008**)
  must pass.
* **Safe production branch** — the production branch is always deployable and protected.
* **Preview per change** — branches/PRs get isolated preview deploys (**0007**) for review.
...
(**0008**) must pass and the branch's preview deploy (**0007**) is used for review;
...
* Good, because it composes directly with the PR-based CI gate (**0008**) and per-branch
  preview deploys (**0007**).
...
via PRs gated by CI (**0008**); preview deploys (**0007**) appear per PR; production tracks
...
* Bad, because that release-management ceremony is disproportionate for a continuously
  deployed app on Vercel (**0007**) with a small team and an agent — overhead with little
  payoff here.

## More Information

Builds on **0008** (PR-gated CI) and **0007** (per-branch preview deploys), which this
workflow assumes. `hotfix/*` branches (off `main`) are the sanctioned exception to the
mandatory coverage gate (**0031**); all other gate steps still apply to them. It is independent of the application stack and could be revisited (e.g.
