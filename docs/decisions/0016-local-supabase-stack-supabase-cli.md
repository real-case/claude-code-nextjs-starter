...
Following the official-CLI principle (**0014**), the project needs a local database to
...
The choice matters for isolation, reproducibility, and testing. End-to-end tests (**0006**)
need a database they can reset and seed without disturbing anyone else, and the migration
workflow (**0011**) is only reproducible if there is a local database to rebuild from
...
* **Reproducibility** — a local database must rebuild deterministically from migrations
  (**0011**).
* **Testability** — e2e tests (**0006**) need a resettable database with a known state.
...
`npm run db:reset` rebuilds it by applying migrations (**0011**). Local Supabase connection
values feed the app's environment in development (wired in **0017**), and e2e tests
(**0006**) run against this stack.
...
tests (**0006**) connect to the local stack via `NEXT_PUBLIC_SUPABASE_*` set from `supabase
...
Builds on **0009** (Supabase baseline), **0011** (migrations applied via `db:reset`), and
**0014** (official-CLI principle); it provides the database that e2e tests (**0006**) depend
