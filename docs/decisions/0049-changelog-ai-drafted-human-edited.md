...
The git workflow (**0029**) promotes `dev` to `main` for production releases, but the
...
convention that no record mandates, and much of the history is agent-authored (**0045**)
with PR descriptions of varying granularity. How does this project produce a changelog?

## Decision Drivers

* **A changelog should exist** — releases to `main` (**0029**) need an auditable,
...
`main`, **0029**), an AI pass reads the merge history and PR descriptions since the last
...
* Good, because the human edit step keeps an accountable voice on outward-facing text
  (consistent with the human gates in **0045**/**0046**).
...
* Bad, because it is one more AI invocation in the release path, with the same external-API
  dependency noted in **0047**.

### Confirmation

`CHANGELOG.md` exists and gains an entry with every `dev → main` release PR; the entry is
part of the reviewed release PR (human-edited per **0046**); drafting tooling (script or
...
* Neutral, because quality tracks PR-description hygiene, which review (**0046**) already
  encourages.
...
Builds on **0029** (the release flow this documents) and **0045**/**0046** (attribution
