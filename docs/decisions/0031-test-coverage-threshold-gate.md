...
The project runs tests (**0006**) inside the CI quality gate (**0008**), and changes reach
the integration/production branches through pull requests (**0029**). But the *presence* of
...
* **Actually merge-blocking** — the threshold must block merge via the CI status check
  (**0008**) and branch protection (**0029**), not be advisory.
* **Minimal added infrastructure** — prefer enforcing through the existing runner (Vitest,
  **0006**) and gate (**0008**) rather than adding external services.
...
(**0006**) with no extra service, and it is genuinely merge-blocking through the existing CI
gate (**0008**) and branch protection (**0029**). Concretely:
...
- **Enforcement:** the failing `test:coverage` step turns the CI gate (**0008**) red; because
  merge requires that check (branch protection, **0029**), a sub-threshold PR cannot merge.
- **Exclusions:** generated files (the Supabase types from **0012**), configuration,
...
- **Hotfix exception:** a PR on a `hotfix/*` branch (branched off `main` per **0029**), or
...
`npm run test` inner-loop runner (**0006**) — and fails below the threshold; CI (**0008**) runs
`test:coverage` as its own step and the check is required for merge (**0029**). The only
...
Builds on **0006** (Vitest, the runner that measures and enforces coverage), **0008** (the CI
gate the failing threshold turns red), and **0029** (branch protection that makes the check
