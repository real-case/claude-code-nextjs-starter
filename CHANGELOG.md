...
<!--
  Genesis baseline (ADR 0080): the public history is squashed to a single commit, so this
  first entry is hand-authored — not machine-derived — and describes what the template
  ships. Finalize the wording and the date below before tagging v0.1.0.
-->
