...
RLS authorization layer (**0010**) has an identity to enforce policies against. With Supabase
adopted as the backend baseline (**0009**) — which explicitly includes Auth — the platform
...
* **Identity for RLS** — the authenticated user id must be the one RLS policies (**0010**)
...
* **Fits @supabase/ssr** — the auth mechanism must integrate with the cookie-based SSR
  session handling already chosen in **0010**.
...
session model that `@supabase/ssr` (**0010**) already handles and makes the Supabase user id
...
* Good, because it reuses the `@supabase/ssr` session handling from **0010** with no extra
  integration layer.
* Good, because the baseline is small enough to test fully (**0006**) yet extends to OAuth
...
* Bad, because authentication is now coupled to Supabase — though this coupling was already
  accepted with the backend baseline (**0009**).

### Confirmation

Auth uses the `@supabase/ssr` clients (**0010**); middleware refreshes the session each
request; RLS policies reference `auth.uid()`; the sign-in/sign-up path is covered by e2e
tests (**0006**). Additional methods (OAuth, magic link) each arrive via their own ADR.
...
Builds on **0009** (Supabase baseline including Auth) and **0010** (the `@supabase/ssr`
