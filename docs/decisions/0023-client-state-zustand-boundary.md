...
TanStack Query (**0022**) owns *server state* — data that originates on the server, is
...
* **Clear separation from server state** — client state must not duplicate or shadow data
  that TanStack Query (**0022**) already owns.
...
- **TanStack Query (0022)** holds anything server-derived — fetched entities, lists,
...
- **Zustand** holds only client-only, ephemeral UI state that should *not* live in the URL:
  ...
  interaction state.
- **URL search params (nuqs, 0032)** hold state that should be shareable, bookmarkable, and
...
- **Optimistic UI lives in the Query cache, not Zustand.** Optimistic updates are applied to
  the TanStack Query cache with rollback on error (**0022**), where optimistic UI is the
...
* Bad, because its boilerplate and ceremony are disproportionate for the small amount of
  purely-client state remaining once TanStack Query (**0022**) owns server state.
...
Builds on **0002** (client modules over the RSC default) and **0022** (the server-state
owner this is bounded against). Together with **0032**, they define the full state
model: server state in Query, URL-owned state in the URL (nuqs, **0032**), and ephemeral
client/UI state in Zustand, with no overlap. Optimistic UI is explicitly Query's job (the
preferred default mutation pattern, **0022**), not Zustand's.
