...
# AI-drafted story state matrices and play functions, with the 0041 human judgment retained

## Context and Problem Statement

**0041** mandates that every exported UI component ships CSF 3 stories covering its
meaningful states, enforced in two tiers: a mechanical existence check in CI and a human
judgment call in PR review for *completeness*. **0037** mandates `play` functions for
...
strict TypeScript (**0003**), variant unions, CSF 3's uniform shape (**0035**) — are
machine-readable. Can drafting and completeness-checking be assisted without surrendering
the judgment **0041** deliberately kept human?

## Decision Drivers

* **The judgment gate stays human** — **0041** chose review judgment over mechanical
  exhaustiveness on purpose; assistance must not quietly convert it into rubber-stamping
  a generator.
* **Authoring cost** — the **0037**/**0041** mandates are the priciest authoring
  obligations in the corpus; their per-component cost decides whether they are sustained
  or resented.
* **Machine-readable state space** — strict types (**0003**) and uniform CSF 3 (**0035**)
  make a drafted matrix *reliable enough to be useful*, which is what separates this from
  generic test generation.
* **Meaningful, not mechanical** — **0041** explicitly rejects exhaustive prop
...
* The agent drafts a per-component state matrix (from prop types, variants, and the
  **0041** state categories) plus `play` function drafts; a PR aid flags uncovered states;
  the human completeness judgment of **0041** is unchanged
...
Chosen option: "AI-drafted matrices and play functions under the unchanged 0041 judgment",
...
For a new or changed component, the agent drafts the state matrix along **0041**'s
...
`play` functions (**0037**) using `@storybook/test`. In review, uncovered states are
...
non-meaningful ones, per **0041**.

### Consequences

* Good, because the completeness judgment gets a concrete artifact to judge against —
  the most error-prone part of **0041**'s enforcement becomes systematic.
* Good, because the marginal cost of the **0037**/**0041** mandates drops substantially,
  protecting their sustainability as the component count grows.
* Good, because drafts are uniform (CSF 3, **0035**) and typed (**0003**), so generated
...
stories; the **0041** existence check and review checklist remain in force unchanged;
sampled `play` functions assert behavior traceable to the PR's stated intent, not just
snapshots of implementation. The **0041** judgment step remains in the PR template.
...
* Bad, because completeness review stays unaided mental work that degrades under load —
  the exact judgment **0041** relies on is the part most likely to be skimped.
...
* Bad, because **0041** explicitly rejected this: exhaustive combinations bury meaningful
  states in noise, and every generated story becomes a maintenance and snapshot burden
  (**0039**, **0043** costs scale per story).

## More Information

Operationalizes assistance for **0041** (state mandate) and **0037** (play functions),
within the forms fixed by **0035** (CSF 3) and **0003** (strict types). Story count
growth interacts with snapshot (**0039**) and visual (**0043**) costs — TurboSnap bounds
the latter. Drafting falls under the agent role in **0045**; review of drafts under
**0046**. Revisit if drafted coverage is found to systematically encode bugs (guardrail
