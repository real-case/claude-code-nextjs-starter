...
The hosting platform (Vercel, **0007**) provides Git-integrated build-and-
...
`build`, and `test:coverage` — the coverage-enforcing test run (**0031**), kept separate from
...
from the fast `npm run test` inner-loop runner (**0006**) — so sub-threshold coverage fails the
gate (**0031**). Branch protection requiring the CI check is verifiable in repository settings.
...
Builds on **0007** (the Vercel hosting model this defers deployment to), **0004** (Node 24
pin reused by CI), **0005** (lint/format commands), and **0006** (the tests the gate runs).
