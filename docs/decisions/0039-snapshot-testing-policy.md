...
its mechanism cannot simply be carried forward. With the engines from **0036**, snapshots can
...
* **Clear boundary** — serialized-DOM snapshots are *not* a substitute for interaction
  (**0037**), accessibility (**0038**), or pixel-level visual regression.
...
stack and avoiding the removed addon. Snapshots are produced in the Vitest addon run (**0036**)
— or, where a bespoke assertion is wanted, via `composeStories` + `toMatchSnapshot` in a
colocated test (**0006**) — capturing the serialized DOM of story states as a net for changes
...
interaction (**0037**) or accessibility (**0038**) testing, and **pixel/visual regression is a
...
* Good, because it reuses Vitest's snapshot mechanism (**0006**/**0034**) and adds no
...
(**0029**, **0008**). Pixel/visual regression is absent here by design.
...
* Bad, because accidental structural changes have no automated backstop, leaving a gap against a
  "most comprehensive" goal that the other modalities (**0037**, **0038**) do not fill.

## More Information

Builds on **0034** / **0036** (the Vitest addon that produces snapshots from stories), **0006**
(the Vitest snapshot mechanism and the `composeStories` fixture path), **0035** (the CSF 3
stories serialized), and is gated in CI alongside the others (**0008**); baseline diffs are
reviewed under the git workflow (**0029**). It explicitly **defers pixel/visual regression**
