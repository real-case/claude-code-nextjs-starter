#!/usr/bin/env node
// scripts/ai/pr-review.mjs — ADR 0047 (AI PR review, advisory), folding in 0050
// (story-matrix drafting hints) and 0051 (semantic-a11y pass). Input = the PR diff +
// the ADR corpus; output = a comment that cites record numbers. Never a required check
// (ADR 0046/0047): a human review is always required and the AI never substitutes.
...
const review = await advise({
  ...
  task:
    ...
    "(ADR 0035/0041/0050 — draft a missing-state matrix if gaps exist) and whether a semantic-a11y " +
    "concern is visible in the markup (focus order, label clarity, state-beyond-color — ADR 0051). " +
    "Do NOT restate the diff. End with a one-line overall recommendation.",
  payload: `\`\`\`diff\n${diff.slice(0, 180000)}\n\`\`\``,
});

const body = `### 🤖 Advisory AI review (ADR 0047 — not a required check; a human review is still required, ADR 0046)\n\n${review}`;
