...
URLs, a sitemap and robots rules, and — because the app is localized (**0027**) — per-locale
...
decided (**0027**). Inconsistent or hand-rolled head management tends to drift, miss canonical/
...
integrates with locale routing (**0027**) for per-locale titles and `hreflang`/canonical
...
URLs and locale alternates are derived from the **0027** locale configuration.
...
canonical and `hreflang` alternates are emitted per locale (**0027**); social-card defaults
...
Builds on **0002** (App Router Metadata API) and **0027** (locale configuration driving
