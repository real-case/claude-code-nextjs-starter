...
| CON-003 | **MCP server toolchain** — `context7`, `figma`, `vercel`, `supabase`, `chromatic` — as the mandated agent/dev tooling baseline the project must provide and use | Project-owner mandate — fixed, not deliberated | 2026-06-10 | [0042](0042-mcp-server-configuration.md) — project-scoped, committed config with secrets handled by env-reference |
