...
This choice underpins the design-token (**0025**) and component-layer (**0026**) decisions:
...
* **Design-token integration** — the theme must be expressible as tokens (**0025**) and
  consumable by the component layer (**0026**).
...
configuration keeps the theme in CSS where design tokens (**0025**) live as custom
...
via tokens (**0025**) and consumed by shadcn/ui components (**0026**).
...
* Good, because the CSS-first `@theme` block keeps the theme and design tokens (**0025**) in
...
* Bad, because it offers no design system or token scale out of the box, so consistency and a
  shared vocabulary must be built by hand — and the chosen component layer (**0026**) targets
...
* Bad, because it adds a build-time toolchain and a different authoring model, and the
  ecosystem/component-layer fit (**0026**) is weaker than Tailwind's.
...
expressed in the `@theme` block are decided in **0025**; the component layer styled with
these utilities is **0026**.
