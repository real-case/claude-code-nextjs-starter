...
The git workflow (**0029**) assumes PR review and **0008** leaves branch protection as a
human-configured setting, but no record states *whose* approval a pull request needs —
in particular whether a PR authored by the AI agent (**0045**) requires a human approver,
and whether an automated AI review (**0047**) may count as that approval. With the agent
...
* **A single accountable gate** — exactly one unambiguous human decision point between
  authored code and the integration branch (**0029**).
* **Concentration of invariants** — the crown-jewel rules (RLS as the authorization
  boundary **0010**, the secret fence **0020**) are cheap to violate in code and expensive
...
* Mandatory human approval for every PR; AI review (**0047**) is a first pass that informs
...
invariants the review protects are concentrated and severe. Branch protection (**0008**)
requires at least one human approval on every PR into the integration branch; the
approver is never the PR's author; agent-authored PRs (labeled per **0045**) are reviewed
by a human like any other. The AI review check (**0047**) runs *before* human review and
...
show a human approver. The **0047** check is not listed among required status checks for
...
Builds on **0029** (the PR flow this gates), **0008** (branch protection as the
enforcement point), **0045** (agent role and PR labeling). **0047** defines the AI first
