...
* **Stack fit** — integrates with Next.js (**0002**) and React 19, and its correctness rests
  on the Rules of React, enforceable by the ESLint setup (**0005**).
...
**0005**) flags violations. This makes manual memoization the exception rather than the norm,
and revises the manual-memoization leanings in **0030** — an in-place refinement while both
...
* Bad, because correctness requires strict adherence to the Rules of React — impure renders,
  ...
  misbehave — so the ESLint rule (**0005**) is essential, not optional.
...
(**0005**) and the gate (**0008** via **0005**) fails on Rules-of-React violations; new code
...
Builds on **0002** (Next.js / React integration) and **0005** (the ESLint rule enforcing the
Rules of React the compiler depends on). It is distinct from **0030**: the React Compiler
handles **memoization** (what to recompute), whereas the concurrency primitives of **0030**
(`useTransition` / `useDeferredValue`) handle **scheduling/priority** (when to render) — the
compiler does not replace them. **0030** is updated to reflect that manual memoization is now
