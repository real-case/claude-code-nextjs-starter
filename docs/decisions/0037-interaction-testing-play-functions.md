...
CSF 3 stories (**0035**) support a `play` function — code that runs after the story renders,
driving interactions (clicks, typing, selection) and asserting the resulting DOM. With the
execution engines from **0036**, a `play` function *is* an executed test (in the browser-mode
...
* **Reuse of Testing Library idioms** — interactions should be written with the same
  user-centric API as the RTL layer (**0006**) to keep one mental model.
* **Callback verification** — handlers passed as props (`onChange`, `onSubmit`, …) must be
  provably called, not assumed.
* **Engine fit** — the standard must run unchanged on both engines in **0036**.
...
* No `play` in stories — interaction tested only in the RTL layer (**0006**), stories stay
  render-only
...
dialogs, and forms (**0019**) — ships at least one story whose `play` function drives the UI
...
executed by the engines in **0036**. **Purely presentational components are exempt** — a render
...
* Good, because interactive behavior is actually exercised — the catalogue proves components
  *work*, and the same `play` runs as a test in both engines (**0036**).
* Good, because callbacks are verified with `fn()` spies, so broken wiring fails a test rather
  than slipping through.
* Good, because `userEvent`/`within` mirror the RTL idioms (**0006**), so there is one
...
**0036** engines and contribute to the **0031** coverage. Presentational-only components carry
...
* Good, because behavioral coverage becomes a guaranteed property of every interactive
  component, executed by **0036**.
* Good, because `fn()` spies verify callbacks and the API matches RTL (**0006**).
...
* Bad, because it forfeits the stories-as-tests reuse (**0034**/**0036**): the interaction
  fixtures and the documentation fixtures diverge, and the test-runner smoke pass exercises
  no behavior.

## More Information

Builds on **0035** (CSF 3 object stories that host `play`), **0036** (the engines that execute
`play`), **0034** (stories-as-tests), **0006** (shared Testing Library idioms and the
RTL boundary), **0003** (strict typing of interaction code), and **0019** (forms are a primary
interaction surface). The accessibility checks layered over the same stories are decided in
**0038**. Targets the Storybook 9 `storybook/test` package; revisit if that API changes.
