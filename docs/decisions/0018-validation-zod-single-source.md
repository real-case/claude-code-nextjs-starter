...
* **Strict-typing synergy** — inferred types must be precise enough to satisfy the strict
  policy (**0003**).
* **Ecosystem fit** — the validator should integrate with the form library (**0019**) and
...
**0019**), request bodies, and external data, and types are always inferred from schemas
...
(**0021**) and form UX (**0019**) — the marker is not a license to surface internals to end
users.
...
* Good, because Zod integrates cleanly with React Hook Form (**0019**) and a Zod-validated
...
interface duplicates a schema. The env module and form validation (**0019**) both build on
...
the forms decision (**0019**) and by the later environment-variables decision; request-body
