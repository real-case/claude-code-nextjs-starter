...
The mandated `figma` server (**CON-003**, configured in **0042**) presupposes a design-handoff
...
has a canonical design-token layer in code (**0025**, CSS custom properties mapped into the
...
role of the `figma` server in the handoff; it does not redesign the token layer (**0025**) or
the component layer (**0026**).

## Decision Drivers

* **One source of truth for design values** — no divergence between Figma and the **0025** token
  layer.
* **Code-first project** — the canonical token layer already exists in the repo (**0025**), and
  the project is decisions-/code-first.
* **Use the mandated figma server productively** — agents and developers should pull design
  *context* from Figma during implementation (**CON-003**/**0042**), read-only.
...
* **Figma-canonical tokens**; a pipeline exports Figma variables (e.g. via Style Dictionary)
  into the **0025** layer, which consumes generated tokens
...
already-decided **0025** token layer as the single source of truth and avoids a generated-token
pipeline, while still using the mandated `figma` server where it adds the most value. The
**0025** CSS-custom-property layer mapped into Tailwind (**0024**) remains canonical. **Figma is
...
least-privilege posture of **0042**) to pull design *context* — component anatomy, states,
...
specs while implementing shadcn components (**0026**) against the canonical tokens (**0025**); a
discrepancy is reconciled either by updating Figma to the code scale or by proposing a token
change through the normal process that touches **0025** — never by a silent fork. Figma component
...
* Good, because there is one source of truth — the repo — so there is no generated-token drift,
  and **0025** stays canonical and unchanged.
...
* Bad, because a token change that originates in design needs a manual round-trip into the
  **0025** layer rather than syncing automatically.
* Bad, because keeping Figma and code component sets named in parity is an ongoing convention
  that can rot without attention.

### Confirmation

The **0025** layer remains the canonical token definition and **no Figma→token generation
pipeline exists**; the `figma` server is configured read-only (**0042**) and used for design
context; Figma uses the shared token scale; Figma component names mirror `src/components/**`;
and token changes flow through the **0025** layer, not around it.

## Pros and Cons of the Options

### Code-canonical tokens, Figma read-only context (chosen)

* Good, because it preserves the single source of truth (**0025**) with no sync pipeline.
* Good, because the `figma` server is used read-only for context, fitting **0042**'s
...
* Bad, because it overturns **0025**'s canonical status, adds a Figma-variables→Style-Dictionary
...
* Bad, because design intent reaches code by eyeballing, guaranteeing drift between Figma and
  **0025** and wasting the mandated `figma` server entirely.

## More Information

Resolves the design-handoff process implied by the mandated `figma` server (**CON-003**,
configured in **0042**). Builds on **0025** (the canonical token layer this keeps as source of
truth), **0024** (the Tailwind theme it maps into), **0026** (the components Figma mirrors), and
**0027** (localized/RTL design considerations). It is distinct from visual regression
