...
Following the official-CLI principle (**0014**), the project must choose how the Next.js app
...
variables**: production env lives in the Vercel project (**0007**, **0020**), and local runs
...
* **Official-CLI alignment** — consistent with **0014**: both `next dev` (the framework's
...
`npm run dev` runs the orchestrated `next dev` path (**0017**); env sync runs through the
...
* Bad, because platform-level behavior that lives outside the framework (e.g. `vercel.json`
  ...
  per-PR preview deployments (**0007**).
...
`npm run dev` invokes the orchestrated `next dev` path (**0017**); the env-sync step
...
* Bad, because env values drift from the Vercel project over time — exactly the "works
  locally, breaks in prod" class the parity principle (**0014**) exists to prevent.

## More Information

Builds on **0014** (official-CLI principle — satisfied here by the framework CLI for the
...
decided in **0017**; the hosting platform whose env this syncs is recorded in **0007**; the
env validation model is **0020**. Vercel's CLI documentation (vercel.com/docs/cli/dev,
