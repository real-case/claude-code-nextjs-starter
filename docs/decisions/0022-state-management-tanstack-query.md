...
data-access layer (**0010**) runs on the server. But interactive client features still need
...
* **RSC stays the default** — initial and server-rendered data continues to come from Server
  Components (**0002**, **0010**); the client cache is for interactive, post-hydration needs.
...
server-rendered data (**0002**, **0010**); TanStack Query handles interactive client data
...
Builds on **0002** (RSC default) and **0010** (server data access). The split between this
server-state cache and purely client-side UI state is drawn in **0023** (Zustand); both are
