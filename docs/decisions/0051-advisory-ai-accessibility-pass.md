...
**0038** gates the build on axe-core at WCAG 2.2 AA over every story state, and is honest
...
* **The uncovered class is the judgment class** — every mechanical a11y check is already
  gated (**0038**); only semantic quality is unprotected.
* **The gate must stay deterministic** — **0038** chose a hard gate because axe results
  are reproducible; probabilistic findings must not block merges (same principle as
  **0047**).
* **Story fixtures already exist** — the **0041** state matrix renders every meaningful
...
* Extend the blocking **0038** gate with AI-evaluated checks
...
probabilistic check can responsibly hold. A CI job renders the **0041** story states
(reusing the built Storybook, **0036**) and submits DOM and screenshots to an AI review
...
tokens, **0025**). Findings are PR comments; the job is not a required check; the **0038**
...
* Good, because the deterministic gate (**0038**) is untouched; the two layers cannot
...
CI contains the advisory a11y job, absent from required checks; the **0038** gate
...
### Extend the blocking 0038 gate with AI checks
...
* Bad, because it puts probabilistic judgments among required checks — flaky merges,
  override machinery, and erosion of trust in the gate (**0008**'s determinism principle,
  the same reasoning that kept **0047** advisory).
...
Extends the accessibility decision of **0038** without modifying its gate; reuses
**0041** states, the built Storybook (**0036**), and tokens (**0025**) for contrast/state
context. Same advisory-authority reasoning as **0047** and the same story-reuse economics
as **0043**. Falls under the agent toolchain posture of **0042**/**0045**. Revisit when
