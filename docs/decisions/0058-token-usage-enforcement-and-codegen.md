...
**0025** made CSS custom properties the canonical design tokens, mapped into Tailwind's
`@theme` (**0024**); **0026** styles components against the semantic names. But **0025**
...
truth **0025** exists to centralize. As the agent becomes the primary implementer (**0045**)
...
enforcement and the single-source codegen that **0024**/**0025** deferred; it does not change
...
* **One source, no laundering** — the CSS `@theme`/`:root` layer (**0025**) is canonical; the
...
* **Reuse the proven `gen:types` shape** — the repo already runs a generate→format→commit→CI-drift
  loop for `database.types.ts` (**0012**); token codegen should mirror it exactly, not invent a
  new pipeline.
...
* **Review-only** — rely on PR review and the **0053** drift audit to catch raw values

## Decision Outcome

Chosen option: "single-source codegen + a two-layer lint gate", because it is the only option that
makes token usage a *guarantee* while keeping the canonical CSS layer (**0025**) the one place a
...
the lint allowlist, wired exactly like `gen:types` (**0012**): styled output through `prettier`,
...
(**0008** deferrals), the lints run in the existing quality gate; nothing new is deferred-out.

### Consequences

* Good, because raw values and invented token names become merge-blocking, closing the silent-typo
  gap **0025** named — the most common agent violation is now caught by a machine, not a reviewer.
...
clean `git status` (no drift), exactly as `gen:types` does (**0012**); a CI step fails on drift. A
...
generated artifact). Subject to the **0053** drift audit once accepted.
...
* Good, because it reuses the **0012** `gen:types` pattern the repo already trusts.
...
* Bad, because token misuse is exactly the high-frequency, low-judgment class that review skips under
  load; **0053** is a periodic backstop, not a per-PR guarantee.

## More Information

Extends **0024** (the CSS-first `@theme` config) and **0025** (the canonical token layer) by adding
the enforcement and codegen they deferred; mirrors **0012** (`gen:types`) for the generate→drift-check
loop and runs in the **0008** CI gate. The "no external margin" invariant is completed mechanically by
...
and P6 (agent rules diverging from CI). Drafted `proposed`; acceptance is the human gate (**0045** /
`adr.py accept`).
