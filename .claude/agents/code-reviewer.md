...
git diff dev...HEAD --stat   # branch/PR scope (dev is the integration branch, 0029)
```
...
- [ ] **Generated file is off-limits** — `src/lib/supabase/database.types.ts` is produced by `npm run gen:types` (0012) and excluded from lint/format. Never hand-edit it; schema changes go through a migration (0011), then regenerate.
...
- [ ] **One package manager: npm** — flag any `pnpm-lock.yaml`, `yarn.lock`, or bun lockfile; `package-lock.json` is the only lockfile (0017).
- [ ] **No publishing** — this is a deployed app, not a package (no `npm publish`).

### 🔒 Security & secrets (critical)

- [ ] **Secrets stay server-only** — only non-secret values may carry the `NEXT_PUBLIC_` prefix. The Supabase service-role key and any API secrets must never reach client code or the repo. No committed `.env*` except `.env.example` (0020). Once the env modules exist, secrets are read only via `src/lib/env.server.ts` (imports `server-only`); no raw `process.env.X` outside `src/lib/env*.ts` (0020).
- [ ] **RLS is the authorization boundary** (0010) — Supabase queries run **as the user** (anon key + the request's session cookie); RLS enforces access. Flag any service-role usage in client-reachable code, or any pattern that bypasses RLS to read/write another user's rows.
- [ ] **Auth/session correctness** (0010, 0013) — in server, proxy, and middleware code use `getClaims()` / `getUser()`, **never `getSession()`** for trust decisions. In the session-refresh path, keep **no logic between** `createServerClient(...)` and the `getClaims()` call — interleaving code there causes the "random logout" bug.
- [ ] **New user-data tables** (0010, 0011) — RLS explicitly enabled (`alter table … enable row level security`) with own-row policies in plain SQL; `SECURITY DEFINER` functions pin `set search_path = ''`. These are human-review gates — surface them, don't wave them through.
- [ ] **MCP config** (0042) — `.mcp.json` carries only `${ENV_VAR}` references, never literal tokens; servers official and version-pinned.
...
- [ ] **Request-scoped Supabase clients** (0010) — the server client is created per request; never hoist `createServerClient` to module scope (it would leak one user's session across requests).
- [ ] **Next 16 specifics** — it's **`src/proxy.ts`, not `middleware.ts`**; `proxy.ts` **composes** next-intl routing with Supabase session refresh (neither replaces the other) (0010, 0013, 0027). `params` / `searchParams` in pages & layouts are async — must be `await`ed or unwrapped with `use()`. Parallel routes (`@modal`, `@*`) need a `default.tsx`.
- [ ] **i18n routing** (0027) — routes live under `src/app/[locale]/…`; locale config is single-sourced (e.g. `src/i18n/routing.ts`). Use the next-intl navigation helpers, not bare `next/link`, for locale-aware links.
...
- [ ] **State buckets** (0022, 0023, 0032) — server state in TanStack Query; ephemeral UI state in Zustand (never mirrors server data, never imported by Server Components); shareable/bookmarkable state in the URL via nuqs. Optimistic state lives in the Query cache, not Zustand.
...
- [ ] **SEO/metadata** (0028) — new routes meet the per-route metadata floor with i18n-aware canonicals.

### 🎨 Styling (Tailwind v4 CSS-first)

- [ ] No `tailwind.config.js`; no runtime CSS-in-JS (0024). Theme tokens belong in `@theme`; components reference semantic tokens, not raw hex values (0025).

### ⚛️ React Compiler (0033) — ENABLED in this project
...
- [ ] For render-bound jank prefer `useTransition` / `useDeferredValue`; debounce/throttle for network costs, virtualization for huge DOM (0030).
...
- [ ] Missing error handling in async code (try/catch, `.catch()`); App Router error boundaries where appropriate (0021).
- [ ] Off-by-one errors, incorrect array/object operations.

### ⚡ Performance

- [ ] Unbounded list rendering without pagination or virtualization (0030).
...
This repository is the starter template. The **infrastructure** ADRs are all wired and their checks are **active**: testing (Vitest/RTL/Playwright, 0006; coverage ≥80%, 0031), Zod validation with the `z.infer`/origin-marker rules (0018), the env modules + `server-only` fence (0020), react-hook-form + `zodResolver` (0019), the Supabase `@supabase/ssr` clients (0009–0013, 0016), next-intl (0027), the TanStack Query / Zustand / nuqs state buckets (0022/0023/0032), the error boundaries + structured logger (0021), and the Storybook circuit (0035–0041, 0057).
...
6. **Run the gates when in doubt** — the quality gate (mirrors CI, 0008): `typecheck → lint → format:check → check:* → build → test:coverage`. All scripts exist; run them directly, e.g. `npm run typecheck`, `npm run lint`, `npm run check:design-system`, `npm run build`.
