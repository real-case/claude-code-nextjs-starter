...
**0027** adopted next-intl with locale-prefixed routing, which gives the project the
...
already in the workflow (**0045**), the question is what the leanest defensible
translation pipeline looks like.

## Decision Drivers

* **Parity, mechanically enforced** — a missing or orphaned key in any locale is a
  defect detectable by structure alone; that class belongs in the CI gate (**0008**), not
  in review judgment.
...
* **One canonical locale** — divergence about which text is authoritative mirrors the
  token problem **0044** solved: there must be a single direction of truth.
...
**0046** — with locale-competent review (native or fluent) required for tone-sensitive
surfaces, and accepted as-drafted for mechanical strings. CI (**0008**) enforces the
...
* Good, because translations ship in the same PR as the strings they translate — no
  lagging-catalog state exists, and locale-prefixed routes (**0027**) never render
...
(**0008**); PRs changing source strings contain the corresponding drafted entries for all
locales; sampled translation PRs show human review; no string authoring happens directly
in non-source catalogs (drift audit **0053** can verify the convention).
...
* Bad, because it adds an external platform, a sync pipeline against in-repo catalogs
  (the brittle-pipeline shape **0044** avoids), cost per word, and turnaround latency
...
Supplies the workflow **0027** presupposed. Direction-of-truth reasoning follows
**0044**; editorial human-gate reasoning follows **0049**; the mechanical/judgment split
follows **0038**/**0051**. Agent drafting falls under **0045**, review under **0046**,
parity enforcement under **0008**. Revisit toward a TMS when locale count, content
