...
**0034** adopted Storybook as the component workbench, with stories doubling as component
...
The format is not cosmetic here. **0034**'s stories-as-tests reuse — `composeStories` /
...
* **Consistency and low boilerplate** — one format across every story file, with minimal
  ceremony per story.
* **Fit with stories-as-tests (0034)** — the chosen format must compose cleanly into the
...
* **Mechanical enforceability (0005)** — the standard should be a lint rule, not a wiki
...
exactly what **0034**'s stories-as-tests reuse (`composeStories`, portable stories) is built
...
config alone does not police the format, so the flat ESLint setup (**0005**) enables
...
rule matching `Template.bind` in story files. Running in the CI quality gate (**0008**),
...
* Good, because object stories drop straight into `composeStories` as tests, so **0034**'s
...
* Good, because `eslint-plugin-storybook` makes the standard mechanical (**0005**, **0008**),
...
are active in the flat ESLint config (**0005**) and run in CI (**0008**); story files use
...
* Good, because object stories are what `composeStories` / the Vitest addon (**0034**) reuse
  as tests directly.
...
* Bad, because each story repeats `Template.bind({})` boilerplate, composes worse, and does
  **not** drop cleanly into the portable-stories test reuse (**0034**) — defeating the main
...
Builds directly on **0034** (the workbench and the stories-as-tests reuse this format serves),
**0003** (the strict-TypeScript / `satisfies` typing pattern stories follow), **0005** (ESLint
flat config that hosts `eslint-plugin-storybook`), and **0008** (the CI gate that runs lint).
CSF 3 is the default authoring format on the Storybook 9 line targeted in **0034**. MDX stays
