...
CLI (**0011**), the schema is the source of truth — but TypeScript does not know about it
...
* **Single source of truth** — types must derive from the SQL schema (**0011**), not be
...
what is checked in. The script is run after every migration (**0011**).
...
* Bad, because it conflicts with the chosen SQL-first migration model (**0011**) and would
  make the ORM, not the SQL schema, the type authority — re-opening a decision already made.

## More Information

Builds on **0003** (strict typing this serves), **0009** (Supabase), and **0011** (the SQL
schema that is the generation source). These generated types are consumed by the data-access
layer (**0010**) and by any feature querying the database.
