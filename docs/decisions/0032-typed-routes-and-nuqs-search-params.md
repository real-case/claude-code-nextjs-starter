...
pagination, selected tab). The current state model covers server cache (**0022**, TanStack
Query) and ephemeral client state (**0023**, Zustand) but has no first-class home for
...
* **A home for URL-owned state** — distinguish state that belongs in the URL from server
  cache (**0022**) and ephemeral client state (**0023**).
...
- **nuqs for search-param state:** use nuqs (`useQueryState` / `useQueryStates`) with typed
  ...
  Zod schema (**0018**), carrying its origin marker.
- **URL as a state bucket:** state that should be shareable, bookmarkable, and survive reload
  ...
  via nuqs. This is the third bucket of the state model, distinct from server cache (**0022**)
  and ephemeral client UI state (**0023**); **0023** is updated so this category lives in the
...
* Good, because the state model is now unambiguous: server state in Query (**0022**),
  URL-owned state in nuqs, ephemeral client state in Zustand (**0023**).
* Bad, because typed routes is a build-time codegen feature (historically behind an
...
**0018**, where validation is needed). The "URL vs Zustand vs Query" bucket rule is
documented (**0022**, **0023**), and misplaced state is caught in code review.
...
the URL boundary), and **0018** (Zod-backed parsers where validation is needed). It refines
the state model of **0022** and **0023** by introducing URL-owned state as a distinct bucket.
Typed routes also covers the localized route segment from the i18n decision (**0027**).
