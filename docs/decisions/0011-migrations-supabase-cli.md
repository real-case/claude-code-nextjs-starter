...
With Supabase/Postgres adopted (**0009**) and authorization living in SQL Row-Level
Security policies (**0010**), the database schema is a first-class, security-relevant
...
types (**0012**). An ad-hoc approach — applying SQL by hand in a dashboard — would leave no
...
unambiguous source of truth for type generation (**0012**). `npm run db:reset` rebuilds the
...
* Bad, because there is no ORM-level type inference here — types come separately from the
  generation step (**0012**), not from the migration tool itself.
...
and exercised by e2e tests (**0006**).
...
Builds on **0009** (Supabase baseline) and **0010** (RLS policies expressed in these
migrations). The generated TypeScript types that consume this schema are decided in
**0012**; the local database tooling that runs `db:reset` is detailed in later
