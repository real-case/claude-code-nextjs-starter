...
The testing stack spans four execution surfaces — Vitest unit/RTL (**0006**), Vitest
browser-mode story tests (**0036**), the test-runner smoke pass over the built Storybook
(**0036**), and Playwright e2e (**0006**) — plus the visual (**0043**) and accessibility
(**0038**) checks, all wired into one gate (**0008**). A failed run on any surface demands
...
* **No laundering** — the gate (**0008**, **0031**) is only meaningful if a pass means a
...
* Good, because every red gate arrives pre-diagnosed, cutting the most tedious
  multi-surface cost of the **0034–0041** testing investment.
...
The triage workflow exists and triggers on failure of gate jobs (**0008**); CI
...
* Bad, because four execution surfaces make log-reading the recurring tax on every red
  build, paid by the scarcest resource (**0046**'s bottleneck argument).
...
* Bad, because it systematically launders intermittent real regressions into green builds —
  directly undermining **0008**/**0031** and everything gated on them.
* Bad, because flakes accumulate silently: the suite degrades while dashboards stay green.

## More Information

Builds on **0008** (the gate it triages), **0006**/**0036** (the surfaces), **0031**/
**0040** (coverage gating that failure-laundering would corrupt), **0043** (animation/
determinism discipline as a flake source). Related: **0047** (AI on diffs pre-merge),
**0053** (scheduled audit that can police quarantine rot). Revisit if triage accuracy
