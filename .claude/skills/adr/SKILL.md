...
using the full [MADR](https://adr.github.io/madr/) template. An ADR records *one*
...
understands not just *what* was decided but *why* — including the roads not taken.
...
- **Replace every `{…}` placeholder** with concrete prose, and **delete the
  ...
  finished record.
- Because this is the *full* template (not minimal), keep and actually populate the
