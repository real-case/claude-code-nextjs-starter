...
**0031** set a single global ≥80% line-coverage threshold gating PR merge, run via
`npm run test:coverage`. That decision predates **0034** (Storybook + the Vitest addon) and
**0036** (two execution engines). The project now has **two Vitest projects** measuring the
same `src/**`: the unit/component project (RTL, **0006**) and the Storybook stories-as-tests
project (**0034**/**0036**). Each exercises different code paths against the same source, so the
**0031** gate is currently ambiguous — it does not say whether the two projects' coverage is
combined into one number, kept as separate thresholds, or whether only one project counts. This
record operationalizes **0031** for the two-project world; it does **not** change the 80% figure.

## Decision Drivers

* **One unambiguous number** — the **0031** gate must mean exactly one thing, not "80% of
  which project?".
...
* **CI fit** — the merged measurement runs non-interactively in the gate (**0008**).

## Considered Options

* Single Vitest workspace, one coverage provider, **merged** coverage across both projects
  against the one ≥80% threshold (**0031**)
* Separate per-project thresholds (e.g. unit ≥X%, stories ≥Y%), no merge
* Only the unit/RTL project counts toward **0031**; story-tests contribute no coverage

## Decision Outcome

Chosen option: "single Vitest workspace with merged coverage", because it keeps the **0031**
gate meaning exactly one thing while crediting coverage from either project. A single Vitest
**projects/workspace** configuration runs both the unit/RTL project (**0006**) and the Storybook
project (**0034**/**0036**) under **one coverage provider — V8 (`@vitest/coverage-v8`)**,
...
**0031**: a line covered by a story-test *or* a unit test counts, and the gate keeps one
definition. Include/exclude is uniform — the stories themselves, generated Supabase types
(**0012**), and config are excluded so they neither pad nor dilute the number. The
**test-runner smoke pass (0036) contributes no coverage**: coverage is single-sourced from the
Vitest projects to avoid instrumentation mismatch and double counting, while the runner stays
responsible for build-integrity smoke. `npm run test:coverage` (**0031**) is defined to run this
merged workspace.

### Consequences

* Good, because there is one coverage number with one meaning, satisfying the **0031** gate
...
* Bad, because a merged line number can mask a weak project — a component covered only by
  ...
  rendered, which is why the breadth mandate is handled separately in **0041**.

### Confirmation

A single Vitest workspace defines both projects; one `coverage` block uses the V8 provider with
a uniform include/exclude (stories, generated types **0012**, and config excluded);
`npm run test:coverage` emits a single merged report checked against ≥80% (**0031**) in CI
(**0008**); the `@storybook/test-runner` job carries no coverage instrumentation.
...
* Bad, because a merged line number can hide an under-tested project (addressed by **0041**).

### Separate per-project thresholds

* Good, because each project's health is visible independently.
* Bad, because it splits the gate into two numbers, reopening exactly the ambiguity **0031**
...
* Good, because it is the simplest — one project, one number, unchanged from a pre-Storybook
  world.
* Bad, because the story-tests (**0034**/**0036**) — a primary reason Storybook was adopted —
  would earn no coverage credit, undercounting real coverage and discouraging stories-as-tests.

## More Information

Operationalizes **0031** (the threshold itself is unchanged) for the two projects introduced by
**0006** (unit/RTL) and **0034**/**0036** (stories-as-tests); excludes generated types from
**0012**; runs in the CI gate (**0008**). This record settles *how much* is measured and how
...
decided in **0041**. Revisit if a per-project visibility need arises (it can be added as
