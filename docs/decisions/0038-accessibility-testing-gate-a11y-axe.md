...
The component layer is built on Radix via shadcn/ui (**0026**), which gives an accessibility
...
the browser-mode Vitest engine from **0036** those results can be made to *fail* the test run.
For the most
...
* **Every state checked** — a11y should be evaluated on each story (each variant/state), reusing
  the catalogue already written (**0035**), not on a hand-picked few.
* **Engine reuse without duplication** — axe should run through an engine already fixed in
  **0036**, not as a separate manual step — and in exactly one of them, honoring **0036**'s
  rule that no assertion suite is duplicated across the two.
* **Contrast ties to tokens** — color-contrast outcomes depend on the design tokens (**0025**)
  being defined to AA, so the gate and the token layer reinforce each other.
* **CI fit** — the check must run non-interactively in the quality gate (**0008**).
...
* No accessibility tooling — rely on Radix's baseline (**0026**) alone
...
configured so that violations **fail** the browser-mode Vitest addon run. Per **0036**, axe
...
**WCAG 2.2 level AA**. Color-contrast checks lean on the design tokens (**0025**) being chosen
...
* Good, because it runs through the single browser-mode engine fixed in **0036**, so a11y
  results live in one place with no duplicated suite to keep in sync.
* Good, because tying contrast to the AA-defined tokens (**0025**) means the gate and the design
...
(**0036**) fails on axe violations while the test-runner config carries no axe hook; the
...
* Good, because a11y becomes enforced on every state, reusing the stories and the
  browser-mode engine from **0036**.
* Good, because it co-designs with the AA design tokens (**0025**).
...
Builds on **0026** (the Radix/shadcn a11y baseline this gate verifies in context), **0034** /
**0036** (the workbench and the browser-mode engine that runs axe over stories), **0025**
(the AA design tokens that drive
contrast outcomes), **0024** (Tailwind that applies them), and **0008** (the CI gate). The
interaction stories the a11y checks run over are standardized in **0037**. The gate targets
