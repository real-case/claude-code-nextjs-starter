...
With Zod established as the validation authority (**0018**) and Vercel managing
per-environment variables (**0007**), the project must decide how env is validated, typed,
...
* **One validation model** — reuse Zod (**0018**) rather than a second validation mechanism.
...
project's existing validator (**0018**) to validate and type all env in one model, fails
...
import fails the build. Values are stored per environment in Vercel (**0007**) and pulled
locally by the dev orchestrator (**0017**).
...
values; the orchestrator (**0017**) pulls them locally.
...
* Bad, because it adds a dependency and its own conventions for what is a small amount of
  project-specific glue we can write directly on Zod (**0018**) with the `server-only` fence.
...
Builds on **0003** (typed access), **0018** (Zod as the validator), **0017** (local env
pull), and **0007** (Vercel as the per-environment store). The Supabase service-role key
governed by the **0010** server-only boundary is one of the secrets fenced here.
