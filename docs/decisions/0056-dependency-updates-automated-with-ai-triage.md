...
lockfile (**0017**), version-pinned MCP servers (**0042**) — but no record decides how
...
the CSS-first config (**0024**/**0025**), a Storybook major the whole **0034–0041**
...
* **The human gate stands** — merges remain human per **0046**; the CI gate (**0008**)
...
* **Bot config is a trust surface** — automated-update configuration executes with repo
  access and must be bounded like the MCP config (**0042**).
...
while keeping the single human gate (**0046**) intact. A Renovate-class bot opens
...
decisions the change touches (e.g. "Tailwind 5: config model changes affect **0024**/
**0025**; if adopted, those records need superseding review"). Humans merge; the CI gate
(**0008**), including story/visual layers (**0036**, **0043**), validates each bump.
MCP server versions (**0042**) are included in the update surface. Auto-merge is
...
* Good, because corpus-impact notes connect the dependency stream to the decision
  ...
  instead of silent drift (**0053**'s concern, fed at the source).
...
* Bad, because the bot configuration and its credentials are an additional committed
  trust surface, mitigated the **0042** way: pinned, least-privilege, reviewed.
...
schedule; merges are human (**0046**); no auto-merge rule exists; MCP server versions
appear in the update surface (**0042**).
...
* Bad, because it waives the single human gate (**0046**) on the exact vector — upstream
...
Supplies the missing motion policy for the pins in **0004**, **0017**, and **0042**.
Builds on **0008**/**0036**/**0043** (the validation each bump runs through), **0046**
(the merge gate), **0001**/**0053** (supersession as the response to
decision-invalidating upgrades). Adjacent: **0055** covers the diff-time security of
first-party code; this record covers the third-party stream; **0052**'s platform-watch
