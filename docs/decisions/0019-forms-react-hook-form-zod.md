...
(**0018**), and the rendering model is RSC-first with Server Actions available for mutations
(**0002**).
...
* **Reuse the Zod schema** — the same schema (**0018**) should validate client input and the
...
form state with `zodResolver`, so a single Zod schema (**0018**) drives client-side
...
Builds on **0002** (RSC/Server Actions) and **0018** (the Zod schemas reused here). Form
validation is covered by the testing strategy (**0006**); richer field components may later
