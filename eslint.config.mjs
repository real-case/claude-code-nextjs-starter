...
const eslintConfig = defineConfig([
  ...
  // ADR 0020: application code reads typed env from the two env modules; raw
    ...
    rules: {
      "no-restricted-properties": [
        "error",
        {
          ...
          property: "env",
          message:
            "Read typed env from src/lib/env.ts (public) or src/lib/env.server.ts (secrets) — ADR 0020.",
        },
      ],
  ...
  // reference the semantic token layer only (@theme/:root, ADR 0024/0025). Scoped to
  ...
  // 0020 env-destructuring guard is re-included here.
  {
    ...
    rules: {
      "no-restricted-syntax": [
        ...
        },
        {
          ...
          message:
            "Raw hex color is banned in components — use a semantic token utility (bg-primary, text-muted-foreground) or var(--color-*); ADR 0058/0025.",
        },
        {
          selector:
            "Literal[value=/\\b(?:rgb|rgba|hsl|hsla|oklch|oklab|lab|lch)\\(/i]",
          message:
            "Raw CSS color function is banned in components — use a semantic token; ADR 0058/0025.",
        },
        {
          selector:
          ...
          message:
            "Tailwind numbered palette is banned in components — use a semantic token utility (bg-primary, text-destructive, …); ADR 0058/0025.",
        },
      ],
    },
  },
  // ADR 0035: stories are CSF 3 only. `flat/csf-strict` enforces the CSF shape and
  ...
  ...storybook.configs["flat/csf-strict"],
  {
    ...
    // `Template.bind({})` idiom, so turn both on explicitly for story files (ADR 0035).
    ...
    rules: {
      ...
      "no-restricted-syntax": [
        "error",
        {
          ...
          message:
            "CSF 2 `Template.bind({})` is banned — author CSF 3 object stories (ADR 0035).",
        },
      ],
    },
  },
  // ADR 0005: Prettier owns formatting — last, so it disables every stylistic
  ...
  globalIgnores([
    ...
    "build/**",
    "next-env.d.ts",
    // Tool output (ADR 0006, 0031, 0034):
    "coverage/**",
    "playwright-report/**",
    "test-results/**",
    "storybook-static/**",
