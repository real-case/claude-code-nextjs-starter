...
baseline, **0009**; Vercel for hosting), and each ships an official CLI that reproduces its
runtime locally.
...
they're orchestrated — follow in **0015**, **0016**, and **0017**.
...
environment synced through `vercel env pull` (**0015**), the local Supabase stack via the
Supabase CLI (**0016**), and the single entrypoint that orchestrates them with npm
(**0017**).
...
**0015**, **0016**, and **0017** make the concrete commands real and reviewable. No bespoke
...
* Bad, because contributors share and can corrupt the same database, offline work is
  impossible, and there is no isolated, reproducible environment for tests (**0006**).

## More Information

Builds on **0001** (ADR practice) and **0009** (the Supabase backend this environment runs
locally). The concrete app runtime, database stack, and orchestration entrypoint are decided
in **0015**, **0016**, and **0017**; the hosting platform whose CLI is used is recorded in
the hosting decision (**0007**).
