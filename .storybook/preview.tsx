...
// ADR 0024/0025: stories render against the real Tailwind build and the semantic
 ...
 * globals.css resolves (ADR 0025: light/dark is a semantic-variable override, not a
 * component change). The class is applied via an effect with cleanup so the dark
 * state never bleeds across stories in the browser-mode test run (ADR 0036).
 */
function ThemeFrame({
  theme,
  children,
 ...
 * browser-mode test run (ADR 0036) and DOM snapshot (ADR 0039) are untouched
 ...
 *   - **Caret** is hidden, so a focused input (the `play` functions, ADR 0037)
...
const preview: Preview = {
  parameters: {
    a11y: {
      // ADR 0038: fail the browser-mode run on any axe violation (not advisory).
      test: "error",
      // Pin the conformance target to WCAG 2.2 AA (ADR 0038); contrast outcomes
      // lean on the AA-defined tokens (ADR 0025).
      options: {
        runOnly: {
          type: "tag",
  ...
  globalTypes: {
    theme: {
      description: "Semantic-token theme (ADR 0025)",
      defaultValue: "light",
      toolbar: {
        title: "Theme",
        icon: "circlehollow",
