...
handling (**0020**), and the local environment sync (**0015**) all bind to it.
...
checks (**0008**, decided next, which keeps those checks in GitHub Actions)? The decision
...
(`vercel env pull`, **0015**) and the env decision (**0020**) build on. Vercel owns
build-and-deploy; the CI gate (**0008**) owns quality checks. Production deploys from the
production branch (`main` — the branch model is recorded in **0029**); every pull request
...
* Good, because integrated env management pairs cleanly with the local env sync (**0015**)
  and the env module (**0020**).
...
production branch (`main`, **0029**) deploys to production. Deployment is handled by Vercel
while `.github/workflows/ci.yml` (**0008**) runs the checks.
...
deploy side of the CI/deploy split that the CI quality gate (**0008**) then builds on. The
environment-variable model stored on this platform is decided in **0020**; the local
synchronization of it is **0015**.
