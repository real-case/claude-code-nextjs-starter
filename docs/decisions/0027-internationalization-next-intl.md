...
the Supabase session refresh already running in middleware (**0010**, **0013**), since both
...
* **Middleware composition** — locale negotiation must compose with the Supabase session
  refresh (**0010**, **0013**), not replace it.
...
composes next-intl's locale handling with the Supabase session refresh (**0010**, **0013**)
...
* Good, because composing in middleware keeps the auth-session refresh (**0010**, **0013**)
...
Components. Locale routing is covered by e2e tests (**0006**).
...
**0010** and **0013**. Per-locale page metadata (canonical/hreflang alternates) is handled in
the SEO decision **0028**.
