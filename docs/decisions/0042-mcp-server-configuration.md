...
trust boundary*, which intersects the secrets fence in **0020**.
...
* **No secrets in version control** — a committed config must never carry tokens; the
  server-only secret fence (**0020**) must stay intact.
...
secret fence (**0020**) intact. A single project-scoped configuration file (`.mcp.json` at the
...
untracked local environment (`.env*` / local settings) per **0020**. Servers are **pinned** to
...
* Good, because secrets stay out of version control behind env-references (**0020**), so the
...
* Bad, because contributors must provision the referenced environment variables locally before
  the servers work — a setup step and a documentation burden on onboarding (**0014**, **0017**).
...
**no literal tokens** — only `${...}` references — which a secret scan (**0020**) can verify;
the required environment variables are documented for onboarding (**0014**, **0017**); server
...
* Good, because the mandated toolchain ships in the repo, identical for everyone and
  reproducible.
* Good, because env-references keep all secrets out of version control (**0020**).
...
* Bad, because it writes Vercel/Supabase/Chromatic/Figma tokens into version control — a direct
  breach of the secret fence (**0020**) and an immediate leak on any clone or fork. Rejected.
...
**0020** (the secret fence that makes credentials env-references), **0007** (Vercel, the
`vercel` server's target platform), **0009** / **0010** / **0011** (Supabase, the `supabase`
server's target), and **0014** / **0017** (local environment and onboarding, where contributors
provision the env vars). **Scope note:** configuring the `chromatic` and `figma` servers does
**not** by itself decide the *visual-regression strategy* — explicitly deferred in **0039** —
