...
(**0042**), "no image baselines are committed to the repo" (**0043**), "no Figma→token
generation pipeline exists" (**0044**), "CI commands equal the local commands"
(**0008**). Once records are accepted and code accumulates, silent divergence between
...
* **Mixed predicate types** — Confirmations span mechanical checks ("no literal tokens")
  and judgment checks ("stories cover meaningful states", **0041**); the mechanism must
  handle both.
...
verification backstop by **0048** (quarantine rot), **0052** (absence-of-machinery
confirmation), and **0055** (whole-repo complement to diff-scoped checks). Falls under
the agent role of **0045**; disposition authority under **0046**. Revisit scoping
