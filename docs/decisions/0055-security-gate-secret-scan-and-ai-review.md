...
authorization boundary that request-path code must never bypass (**0010**), the
`server-only` secret fence (**0020**), and env-reference-only credentials in committed
configuration (**0042**, which already assumes "a secret scan (0020) can verify").
...
* **The invariants are recorded and specific** — the highest-value checks here are not
  generic CWE patterns but the corpus's own rules (**0010**, **0020**, **0042**), which
  only a corpus-aware reviewer can check.
* **Determinism sets authority** — the corpus's standing principle (**0008**, **0038**,
  **0047**): reproducible checks may block; probabilistic ones advise.
...
* **Diff-scope for the gate, whole-repo elsewhere** — per-PR checks must stay fast;
  repository-wide assurance has its own mechanism (**0053**).
...
scanner (gitleaks-class) runs in the required gate (**0008**) over the diff and incoming
history, fulfilling the verification **0020** and **0042** already presuppose; findings
block merge. **Layer 2 (advisory):** an AI security pass — a security-focused complement
to **0047** — reviews the PR diff specifically against the recorded invariants: privileged
(service-role) Supabase clients reachable from request paths (**0010**), imports crossing
the `server-only` fence or secrets accessed outside the validated env modules (**0020**),
literal credentials or weakened env-references in `.mcp.json` and config (**0042**),
RLS-disabled tables in migrations (**0011**). Findings are PR comments citing the
...
(**0053**), keeping the per-PR gate fast.

### Consequences

* Good, because the unforgiving class (committed secrets) is blocked deterministically
  pre-merge — the corpus's most-cited verification gap (**0020**, **0042**) closes.
* Good, because the checks generic tooling cannot perform — the project's own
  ...
  citations a reviewer can act on (**0046**).
...
* Bad, because diff scope structurally misses whole-repo and cross-change issues;
  the mitigation is explicitly delegated (**0053**) and must actually run.
* Bad, because the advisory layer inherits **0047**'s costs — tokens per PR, an external
...
The required gate (**0008**) contains the secret-scan job and branch protection lists it
as a required check; the AI security job exists and is absent from required checks;
sampled findings cite ADR numbers (**0010**, **0020**, **0042**); the scanner allowlist
is committed and changes to it are human-reviewed (**0046**); scanner and action versions
are pinned (**0042**'s pinning posture).
...
* Bad, because it is blind to the project's actual crown jewels: no generic rule knows
  ...
  (**0010**) or that `.mcp.json` must contain only `${...}` references (**0042**).
* Bad, because alone it provides no secret scanning, leaving the corpus's assumed
  verification (**0020**, **0042**) still unimplemented.
...
Implements the verification presupposed by **0020** and **0042**; protects the
authorization model of **0010**/**0011**. Advisory-layer mechanics and authority
reasoning follow **0047**; whole-repo complement is **0053**; review disposition falls
under **0046**. Dependency-side supply-chain risk is the adjacent record **0056**.
